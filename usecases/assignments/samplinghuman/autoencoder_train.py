...
for step in range(TRAINING_STEPS):
    ...
    batch_loss = batch_bce_loss  # + fft_mse_loss
    batch_loss.backward()
    optimizer.step()
    cosine_scheduler.step() if step > WARMUP_ITERATIONS else warmup_scheduler.step()
    if step % TRAINING_LOGGING_FREQUENCY == 0:
        logger.info(f"Step {step}, BCE: {batch_bce_loss.item():.4f}")  # , FFT2 MSE: {fft_mse_loss.item():.4f}, Total loss: {batch_loss.item():.4f}")
    ...
    if step % VALIDATION_FREQUENCY == 0:
        ...
        logger.info(f"Validation: step {step}, BCE: {val_batch_bce_loss:.4f}")  # , FFT MSE: {val_fft2_mse_loss:.4f}, total loss: {val_loss:.4f}")
