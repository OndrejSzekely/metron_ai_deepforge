...
val_iterations_num = encoder_val_dataset.get_val_data_images_num() // BATCH_SIZE
...
for step in range(TRAINING_STEPS):
    ...
    if step % VALIDATION_FREQUENCY == 0:
        ...
        val_log.flush()
        # val_visu = visualize_batch(fragment_size=TILE_SIZE, batch=x_val)
        # cv.imwrite(visu_dir + f"/batch_visu_{step}_val.jpg", val_visu)
        # train_visu = visualize_batch(fragment_size=TILE_SIZE, batch=x_train)
        # cv.imwrite(visu_dir + f"/batch_visu_{step}_train.jpg", val_visu)
