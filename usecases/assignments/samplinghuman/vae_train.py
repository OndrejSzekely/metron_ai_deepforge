...
warmup_scheduler = torch.optim.lr_scheduler.LinearLR(optimizer, start_factor=0.005, total_iters=WARMUP_ITERATIONS)
...
for step in range(TRAINING_STEPS):
    ...
    x_train = (
        ...
        .reshape(BATCH_SIZE * (IMAGE_SIZE // TILE_SIZE) ** 2, IMAGE_CHANNELS, TILE_SIZE, TILE_SIZE)
        .clamp(0.0, 1.0)
        .to("cuda")
    )
    ...
    batch_kl_loss = (
        KL_normal_loss(x_train_mean, x_train_logvar) * (step / TRAINING_STEPS) ** 2 if step > WITHOUT_KL_ITERS else torch.Tensor([0.0]).to("cuda")
    )
    batch_loss = batch_bce_loss + fft_mse_loss + batch_kl_loss
    batch_loss.backward()
    torch.nn.utils.clip_grad_norm_(image_encoder_decoder_model.parameters(), 5.0)
    optimizer.step()
    # cosine_scheduler.step() if step > WARMUP_ITERATIONS else warmup_scheduler.step()
    if step % TRAINING_LOGGING_FREQUENCY == 0:
        logger.info(
            f"Step {step}, BCE: {batch_bce_loss.item():.4f}, FFT2 MSE: {fft_mse_loss.item():.4f}, KL: {batch_kl_loss.item():.4f}, total loss: {batch_loss.item():.4f}"
        )
    ...
    if step % VALIDATION_FREQUENCY == 0:
        ...
        for val_step in range(val_iterations_num):
            ...
            val_loss_kl += KL_normal_loss(x_val_mean, x_val_logvar).item() * (step / TRAINING_STEPS) ** 2 if step > WITHOUT_KL_ITERS else 0.0
        ...
        logger.info(
            f"Validation: step {step}, BCE: {val_batch_bce_loss:.4f}, FFT MSE: {val_fft2_mse_loss:.4f}, loss KL: {val_loss_kl:.4f}, total loss: {val_loss:.4f}"
        )
