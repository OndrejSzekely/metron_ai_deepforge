...
def get_body(input):
    ...
    x = tf.keras.layers.BatchNormalization()(x)
    x = get_stage(x, stage_index=3, number_of_blocks=4, channels_num=152, group_width=8, bottleneck_ratio=1)
    x = tf.keras.layers.BatchNormalization()(x)
    x = get_stage(x, stage_index=4, number_of_blocks=7, channels_num=256, group_width=8, bottleneck_ratio=1)
    return x
